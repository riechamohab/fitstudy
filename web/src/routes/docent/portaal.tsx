...
function HealthBadge({ avgLevel }: { avgLevel: number | null }) {
  ...
  return (
    <span
      className={`flex w-fit items-center gap-1 rounded-full px-2.5 py-1 text-xs font-semibold ${meta.bg} ${meta.text}`}
    >
...
function DocentPortaalPage() {
  ...
  /*
   * De backend bepaalt welke studenten bij de ingelogde docent horen.
   * De frontend filtert daarna alleen nog op naam en klas.
   */
  useEffect(() => {
    async function load() {
      try {
  ...
  /*
   * Meldingen ophalen en iedere 30 seconden vernieuwen.
   */
  useEffect(() => {
    let cancelled = false;
    ...
    async function loadNotifications() {
      try {
        ...
        // Meldingen zijn niet essentieel voor het portaal.
      }
    }
  ...
  /*
   * Sluit het meldingenmenu wanneer buiten het menu wordt geklikt.
   */
  useEffect(() => {
  ...
  /*
   * Details van een geselecteerde student ophalen.
   */
  useEffect(() => {
  ...
  /*
   * Alleen klassen die daadwerkelijk in de studentenlijst zitten.
   *
   * Omdat de backend alleen studenten van de ingelogde docent teruggeeft,
   * bevat deze lijst automatisch alleen de klassen van die docent.
   */
   ...
   * Zoek- en klasfilter.
   *
   * Er wordt hier NIET meer gecontroleerd op teacherClasses.
   * De backend heeft al bepaald welke studenten deze docent mag zien.
   */
  ...
  const selectedStudentSummary = students.find(
      ...
      <header className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500">
            Overzicht van je studenten, voortgang en welzijn.
          </p>
        </div>
          ...
          >
            <div className="text-right">
              ...
              <p className="text-xs text-slate-400">
                {(profile as unknown as { subject?: string })?.subject ||
                  "Docent"}
              </p>
            </div>
              ...
              <div>
                ...
                <p className="text-xs text-slate-500">
                  ...
                  {details?.student.className ??
                    selectedStudentSummary?.className ??
                    "—"}
                </p>
              </div>
                    ...
                    <div className="flex items-center justify-between rounded-xl border border-slate-200 bg-slate-50 p-4">
                      <div>
                        <p className="text-sm font-medium text-slate-800">
                          Gemiddeld stressniveau:{" "}
                          {details.stressStats.entries > 0
                            ? details.stressStats.avgLevel
                            : "—"}
                        </p>

                        <p className="mt-0.5 text-xs text-slate-500">
                          Gebaseerd op {details.stressStats.entries}{" "}
                          registratie(s).
                        </p>
                      </div>
                    ...
                    <div className="rounded-xl border border-slate-200 bg-slate-50 p-4">
                      <div className="mb-2 flex justify-between text-sm">
                        ...
                        <span className="font-semibold text-slate-900">
                          {details.taskStats.completed} van{" "}
                          {details.taskStats.total} taken
                        </span>
                      </div>
